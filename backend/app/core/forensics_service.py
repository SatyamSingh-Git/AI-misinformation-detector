...
# Load the AI vs Human image detector model
print("Loading AI vs Human image detector in forensics service...")
try:
    ...
    ai_detector_processor = AutoImageProcessor.from_pretrained(str(model_path))
    ai_detector_model = AutoModelForImageClassification.from_pretrained(str(model_path))
    print(f"AI vs Human detector loaded from: {model_path}")
except Exception as e:
    print(f"Could not load AI detector in forensics service: {e}")
    ai_detector_processor, ai_detector_model = None, None
...
class ForensicsService:
    ...
    def _analyze_with_ai_detector(self, image: Image.Image) -> dict:
        """
        Uses the fine-tuned SigLIP model to classify if the image is AI-generated or human-created.
        Returns classification results with confidence scores.
        """
        if not ai_detector_processor or not ai_detector_model:
            return {"error": "AI detector model not available."}
        
        try:
            # Preprocess the image
            inputs = ai_detector_processor(images=image, return_tensors="pt")
            ...
            # Model labels: 0 = "ai", 1 = "hum" (human)
            is_ai = predicted_class_idx == 0
            label = "AI-Generated" if is_ai else "Human-Created"
            ...
            return {
                ...
                "probabilities": {
                    "ai": probabilities[0][0].item(),
                    "human": probabilities[0][1].item()
                }
            }
        except Exception as e:
            print(f"Error in AI detector analysis: {e}")
            return {"error": f"AI detection failed: {e}"}
    ...
    def analyze_image_authenticity(self, image_bytes: bytes, source_context: str) -> dict:
        ...
        now using the user-provided source context and the AI detector model.
        """
        try:
        ...
        ai_detector_result = self._analyze_with_ai_detector(image)
        metadata_result = self._analyze_metadata(image)
        vision_result = self._analyze_with_gemini_vision(image)

        # --- CONTEXT-AWARE SYNTHESIS ---
        # Primary verdict comes from AI detector model
        ai_classification = ai_detector_result.get("classification", "Error")
        ai_confidence = ai_detector_result.get("confidence", 0.0)
        is_ai_generated = ai_detector_result.get("is_ai_generated", False)
        
        # Determine final verdict based on AI detector
        if is_ai_generated:
            final_verdict = "Likely AI-Generated"
        else:
            final_verdict = "Likely Real Photograph"
        
        explanation_parts = []
        
        # Add AI detector explanation
        if "error" not in ai_detector_result:
            ai_prob = ai_detector_result.get("probabilities", {})
            ai_explanation = f"AI Detection Model: {ai_classification} with {ai_confidence:.1%} confidence (AI: {ai_prob.get('ai', 0):.1%}, Human: {ai_prob.get('human', 0):.1%})."
            explanation_parts.append(ai_explanation)
        
        # Add Gemini vision analysis if available
        if "reasoning" in vision_result:
            explanation_parts.append(f"Visual Analysis: {vision_result['reasoning']}")

        # Metadata analysis with context awareness
        context_flag = ""
        ...
        if source_context == 'camera' and not has_exif:
            ...
            context_flag = metadata_result["flag"]

        explanation_parts.append(context_flag)

        # Calculate final confidence combining AI detector and context
        final_confidence = ai_confidence
        
        # Adjust confidence based on metadata context mismatch
        if source_context == 'camera' and not has_exif and is_ai_generated:
            final_confidence = min(1.0, final_confidence * 1.15)  # Boost confidence for AI detection
            explanation_parts.append("Confidence increased due to metadata absence matching AI-generated prediction.")
        elif source_context == 'camera' and not has_exif and not is_ai_generated:
            final_confidence = max(0.3, final_confidence * 0.7)  # Reduce confidence for real photo without EXIF
            explanation_parts.append("Confidence reduced due to missing camera metadata for claimed real photograph.")

        return {
            "verdict": final_verdict,
            "confidence": final_confidence,
            "full_explanation": " ".join(explanation_parts),
            "ai_detector_analysis": ai_detector_result,
